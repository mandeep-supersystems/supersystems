// ─── WAREHOUSE MODULE: USER ACCESS MANAGEMENT ───
const WH_SECTIONS = [
    { id: 'overview',       label: 'Overview',        icon: 'dashboard' },
    { id: 'zones',          label: 'Warehouse Zones', icon: 'grid_view' },
    { id: 'bins',           label: 'Bin Management',  icon: 'inventory_2' },
    { id: 'picklists',      label: 'Pick Lists',      icon: 'checklist' },
    { id: 'putaway',        label: 'Putaway Tasks',   icon: 'unarchive' },
    { id: 'receiving',      label: 'Goods Receipt',   icon: 'move_to_inbox' },
    { id: 'packing',        label: 'Packing Lists',   icon: 'inventory' },
...
const WH_ENTITIES = [
    { id: 'zones',            label: 'Warehouse Zones' },
    { id: 'bins',             label: 'Bins & QR Codes' },
    { id: 'pick_lists',       label: 'Pick Lists' },
    { id: 'putaway',          label: 'Putaway Tasks' },
    { id: 'receiving',        label: 'PO Goods Receipt' },
    { id: 'shipping',         label: 'Shipping & Packing' },
    { id: 'user_management',  label: 'User Access Management' }
];

const WH_ACTIONS = ['view', 'create', 'edit', 'delete', 'export', 'import'];

const WH_ROLE_DEFAULT_SECTIONS = {
    module_admin: WH_SECTIONS.map(s => s.id),
    editor:       ['overview', 'zones', 'bins', 'picklists', 'putaway', 'receiving', 'packing', 'shipping', 'auditlogs'],
    viewer:       ['overview', 'bins', 'shipping']
};

const WH_ROLE_DEFAULT_PERMS = {
    module_admin: () => { const p = {}; WH_ENTITIES.forEach(e => p[e.id] = [...WH_ACTIONS]); return p; },
    editor: () => { const p = {}; WH_ENTITIES.forEach(e => p[e.id] = ['view', 'create', 'edit', 'export', 'import']); p['user_management'] = ['view']; return p; },
    viewer: () => { const p = {}; WH_ENTITIES.forEach(e => p[e.id] = ['view', 'export']); return p; }
};

function renderSectionCheckboxes(containerId, checkedSections) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = WH_SECTIONS.map(s => `
    ...
    container.innerHTML = `
        <table class="data-table" style="font-size:12px;">
            <thead>
                <tr>
                    <th>Entity</th>
                    ${WH_ACTIONS.map(a => `<th style="text-align:center;">${a}</th>`).join('')}
                </tr>
            </thead>
            <tbody>
                ${WH_ENTITIES.map(e => {
...
function onMuRoleChange() {
    ...
    renderSectionCheckboxes('muSectionCheckboxes', WH_ROLE_DEFAULT_SECTIONS[role] || []);
    renderPermMatrix('muPermMatrix', WH_ROLE_DEFAULT_PERMS[role]());
}

function onEmuRoleChange() {
    const role = document.getElementById('emuRole').value;
    renderSectionCheckboxes('emuSectionCheckboxes', WH_ROLE_DEFAULT_SECTIONS[role] || []);
    renderPermMatrix('emuPermMatrix', WH_ROLE_DEFAULT_PERMS[role]());
}
...
async function loadModuleUsers() {
    ...
    try {
        ...
        tbody.innerHTML = data.data.map(u => {
            ...
            const sections = (perms && perms.sections) ? perms.sections : WH_ROLE_DEFAULT_SECTIONS[u.role] || [];
...
function openEditModuleUser(encodedData) {
    ...
    const sections = u.permissions.sections || WH_ROLE_DEFAULT_SECTIONS[u.role] || [];
    renderSectionCheckboxes('emuSectionCheckboxes', sections);
    renderPermMatrix('emuPermMatrix', u.permissions.entity_permissions || WH_ROLE_DEFAULT_PERMS[u.role]());
